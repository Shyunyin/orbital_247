...
class NonFixedTask extends OneTimeTask {
    ...
    constructor(taskName, taskCategory, month, date, numOfSess, durOfSess, taskAfterIt) {
        ...
        this.accumulatedDuration = [this.numOfSess * this.durOfSess[0], this.numOfSess * this.durOfSess[1]]; // assuming 0 is hours and 1 is mins
        this.group = -1;
    }
            ...
            if (index < 8) {
                ...
                for (i = 0; i < currArr.length; i++) {
                    if (currArr[i].getTaskName == this.taskAfterIt) {
                        ...
                        let newIndex = 0;
                        currArr = Window.prototype.nonFixedCollection[index];
                        while (currArr[newIndex].getTaskName() != this.taskAfterIt) {
                            newIndex++;
                        }
                        let startTime = currArr[newIndex].getStartTimeIN(); //This method needs to be created
                    }
                }
